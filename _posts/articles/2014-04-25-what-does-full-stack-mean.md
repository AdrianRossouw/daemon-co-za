...
Mostly when people call themselves full-stack, they mean in this way; the narrowest definition of the term. This almost indistinguishable from what we know as "web developers", which dillutes the original meaning.
...
You don't need to be the world's best designer, but you need to able to visualize an outcome and then be able to express it in code. To be able implement your designs, you are going to need more than a glancing familiarity with foundations of the browser environment.
...
understand that they have different strength and weaknesses, trying to jam every type of data into the same container is only going to lead to problems in the long term.
...
There is a probably a deeper philosophic principle at play here, about the point where any term becomes so vague that it ceases to have any meaning, or so specific that it doesn't allow people with different perspectives to communicate about complex subjects.
...
This is not a new idea either. There is a much older term called ["T-Shaped Skills"](http://en.wikipedia.org/wiki/T-shaped_skills), that are defined a having a deep specialization in one
